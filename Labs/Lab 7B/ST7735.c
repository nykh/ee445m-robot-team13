...
static Sema4Type LCDFree;       // used for mutual exclusion
...
void ST7735_PlotPoint(long y){long j;
	static char currentPoints[138] = {0};
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  ...
  j = 20+(90*(Ymax-y))/Yrange;
  if(j<20) j = 20;
  if(j>109) j = 109;
	ST7735_DrawPixel(X,currentPoints[X-21],ST7735_BLACK);
  ST7735_DrawPixel(X, currentPoints[X-21] = j, ST7735_WHITE) ;
...
long j;
  ...
  if(j<20) j = 20;
  if(j>109) j = 109;
  ST7735_DrawFastVLine(X, j, 109-j, ST7735_BLUE);
}
...
//Private function Function Protypes
static void longToString (char * buffer, long value);


//------------ST7735_Message------------
// Prints a string to LCD
// Requires device number, line number, input string, and an optional integer value
// Input: device number can be 0 or 1
...
void ST7735_Message (int device, int line, const char *string, long value){
	...
	char valueBuff[25];
	char *sp = 0;
	if (device < 0 || device > 2 || line < 0 || line >7 ){			//Check if input is valid
		return;
	}
	for (i=0; string[i];i++){
		valueBuff[i] = string[i];
	}
	valueBuff[i++] = ' ';
...
// Input: Character pointer should point to the charachter array that holds the result of conversion
//				pass the integer as second parameter
// Output: Input array would be updated
static void longToString (char * buffer, long value) {
	int i=0,digits=1,k=0;
	long _value;
	if (value < 0){																		// Put a negative sign in case of negative input, 
	  buffer[i++] = '-';
		value = -value;
	}
	_value = value;	
	while (_value /= 10){															//Convert the absolute value of input to string
		digits++;
	}
	for (k=0 ; k< digits;k++){
		buffer[i+digits-k-1] = (value % 10) + '0';
		value /= 10;
	}
	buffer[i+digits] = '\0';
}
