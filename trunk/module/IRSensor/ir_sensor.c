...
static FilterType filter1;
filter1.index = FILTER_LENGTH-1; // to let pre-increment start at x[FILTER_LENGTH]
...
static long Filter(FilterType *f, long data) {
  long y = 0;
  unsigned char i;
  
  if(++f->index == 2*FILTER_LENGTH) f->index = FILTER_LENGTH;
  f->x[n] = f->x[n-FILTER_LENGTH] = data;
...
static void Consumer(void) {	
  ...
  while (1) {
		...
		long data = IR1Fifo_Get();
			
		// Choosing whether to apply the filter
		if(Filter_Use == NOW_USING) data = Filter(&filter1, data);
		else if(Filter_Use == STOP_USING) {
			Filter_Use = NOT_USING;
			Filter_ClearMemo(&filter1);
		}
