...
static void myADCInit(unsigned char prescale, unsigned short period) {
  ...
  GPIO_PORTE_AMSEL_R |= PE3_0_M; // 6.0) enable analog functionality on PE3-0
	...
	ADC0_SSPRI_R = 0x3210;              // 10) Sequencer 2 is lowest priority
	...
	// clear SS2 trigger select field
	...
	ADC0_SSCTL2_R = (0                        // settings for 1st sample:
					 ...
					 & ~ADC_SSCTL2_TS1        // read pin specified by ADC0_SSMUX2_R
					 & ~ADC_SSCTL2_IE1         // raw interrupt asserted here
					 & ~ADC_SSCTL2_END1        // sample is end of sequence (hardwired)
					 & ~ADC_SSCTL2_D1       // differential mode not used
					 
					 & ~ADC_SSCTL2_TS2        // read pin specified by ADC0_SSMUX2_R
					 & ~ADC_SSCTL2_IE2         // raw interrupt asserted here
					 & ~ADC_SSCTL2_END2        // sample is end of sequence (hardwired)
					 & ~ADC_SSCTL2_D2       // differential mode not used
					 
					 & ~ADC_SSCTL2_TS3        // read pin specified by ADC0_SSMUX2_R
					 | ADC_SSCTL2_IE3         // raw interrupt asserted here
					 | ADC_SSCTL2_END3        // sample is end of sequence (hardwired)
					 & ~ADC_SSCTL2_D3);       // differential mode not used
