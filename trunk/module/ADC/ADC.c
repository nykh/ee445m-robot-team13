...
static void switchBackToSWTrigger(void);

#define ADC_IDLE       0
#define ADC_COLLECTING 1
static unsigned char ADC_Collect_Flag = ADC_IDLE;

#define ADC_SW_TRIGGER    0
#define ADC_TIMER_TRIGGER 1
static void ADC_Init(unsigned char channelNum, unsigned char mode, unsigned char prescale, unsigned short period);
...
#define NOT_INITIALIZED 12
static unsigned char CurrentChannel = NOT_INITIALIZED;
static unsigned char CurrentCollectChannel = NOT_INITIALIZED;
static unsigned short * CollectBufferPt;
static unsigned int CollectNumberOfSamples;



void ADC_Collect(unsigned int channelNum, unsigned int fs, unsigned short buffer[], unsigned int numberOfSamples){
	...
	// raised semaphore
	ADC_Collect_Flag = ADC_COLLECTING;
	
	// Set the buffer pointer and number of samples
	CollectBufferPt = buffer;
	CollectNumberOfSamples = numberOfSamples;
	
  DisableInterrupts();
	ADC_Init(channelNum, ADC_TIMER_TRIGGER,  prescale, period);
  EnableInterrupts();
}


void ADC_Open(unsigned char channelNum){
  ADC_Init(channelNum, ADC_SW_TRIGGER, 0, 0);
...
static void ADC_Init(unsigned char channelNum, unsigned char mode, unsigned char prescale, unsigned short period) {
  ...
  delay = SYSCTL_RCGCGPIO_R;
	...
	switch(CurrentChannel) {
      ...
      GPIO_PORTE_AFSEL_R &= ~0x08; // 4.0) disable alternate function on PE3
      GPIO_PORTE_AMSEL_R &= ~0x08; // 6.0) disable analog functionality on PE3
      break;
    case 1:                       //      Ain1 is on PE2
      GPIO_PORTE_AFSEL_R &= ~0x04; // 4.1) disable alternate function on PE2
      GPIO_PORTE_AMSEL_R &= ~0x04; // 6.1) disable analog functionality on PE2
      break;
    case 2:                       //      Ain2 is on PE1
      GPIO_PORTE_AFSEL_R &= ~0x02; // 4.2) disable alternate function on PE1
      GPIO_PORTE_AMSEL_R &= ~0x02; // 6.2) disable analog functionality on PE1
      break;
    case 3:                       //      Ain3 is on PE0
      GPIO_PORTE_AFSEL_R &= ~0x01; // 4.3) disable alternate function on PE0
      GPIO_PORTE_AMSEL_R &= ~0x01; // 6.3) disable analog functionality on PE0
      break;
    case 4:                       //      Ain4 is on PD3
      GPIO_PORTD_AFSEL_R &= ~0x08; // 4.4) disable alternate function on PD3
      GPIO_PORTD_AMSEL_R &= ~0x08; // 6.4) disable analog functionality on PD3
      break;
    case 5:                       //      Ain5 is on PD2
      GPIO_PORTD_AFSEL_R &= ~0x04; // 4.5) disable alternate function on PD2
      GPIO_PORTD_AMSEL_R &= ~0x04; // 6.5) disable analog functionality on PD2
      break;
    case 6:                       //      Ain6 is on PD1
      GPIO_PORTD_AFSEL_R &= ~0x02; // 4.6) disable alternate function on PD1
      GPIO_PORTD_AMSEL_R &= ~0x02; // 6.6) disable analog functionality on PD1
      break;
    case 7:                       //      Ain7 is on PD0
      GPIO_PORTD_AFSEL_R &= ~0x01; // 4.7) disable alternate function on PD0
      GPIO_PORTD_AMSEL_R &= ~0x01; // 6.7) disable analog functionality on PD0
      break;
    case 8:                       //      Ain8 is on PE5
      GPIO_PORTE_AFSEL_R &= ~0x20; // 4.8) disable alternate function on PE5
      GPIO_PORTE_AMSEL_R &= ~0x20; // 6.8) disable analog functionality on PE5
      break;
    case 9:                       //      Ain9 is on PE4
      GPIO_PORTE_AFSEL_R &= ~0x10; // 4.9) disable alternate function on PE4
      GPIO_PORTE_AMSEL_R &= ~0x10; // 6.9) disable analog functionality on PE4
      break;
    case 10:                      //       Ain10 is on PB4
      GPIO_PORTB_AFSEL_R &= ~0x10; // 4.10) disable alternate function on PB4
      GPIO_PORTB_AMSEL_R &= ~0x10; // 6.10) disable analog functionality on PB4
      break;
    case 11:                      //       Ain11 is on PB5
      GPIO_PORTB_AFSEL_R &= ~0x20; // 4.11) disable alternate function on PB5
      GPIO_PORTB_AMSEL_R &= ~0x20; // 6.11) disable analog functionality on PB5
      break;
		default:
			break;
	}
...
void ADC0Seq3_Handler(void){
  ...
  ADCvalue = ADC0_SSFIFO3_R&ADC_SSFIFO3_DATA_M;
	if (CollectNumberOfSamples-- > 0){
		*CollectBufferPt++ = ADCvalue;
	} else {
		switchBackToSWTrigger();
		ADC_Collect_Flag = ADC_IDLE;
	}
}

static void switchBackToSWTrigger() {
	switch(CurrentCollectChannel) {
		case 0:                       //      Ain0 is on PE3
      GPIO_PORTE_AFSEL_R &= ~0x08; // 4.0) disable alternate function on PE3
      GPIO_PORTE_AMSEL_R &= ~0x08; // 6.0) disable analog functionality on PE3
      break;
  ...
  NVIC_DIS0_R |= NVIC_EN0_INT17;            // disable interrupt 17 in NVIC
  // **** Disable Timer0A ****
  TIMER0_CTL_R &= ~TIMER_CTL_TAEN;          // disable timer0A during setup
  // **** ADC initialization ****
  ADC0_ACTSS_R &= ~0x0008;        // 11) disable sample sequencer 3
