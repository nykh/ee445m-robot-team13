...
// Modified to add high-level feature to display string and numbers
// Siavash Zangeneh, Nicholas Huang
// Jan 16, 2013
...
void ST7735_Message (int device, int line, char *string, long value){
	short int y = device * 80 + line * 20;
	short x=0;
	char valueBuff[15];
	char *sp = 0;
	ST7735_FillRect(0, y, 128, 20, 0);
	if (string){
		sp = string;  	
	} else {
		sp = valueBuff;
		longToString(valueBuff, value);		
	}
	while (*sp){
	  	ST7735_DrawChar(x, y, *sp, ST7735_Color565(255, 128, 128), 0, 2);
		  sp ++;
	  	x+= 12;
	}
}
